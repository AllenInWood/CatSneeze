...
public class FuzzySearchDaoImpl implements IFuzzySearchDao {
    public List<Movies> listMoviesByFuzzySearch(String query, int limit, int offset) {
        ...
        BigDecimal maxWrongWord = new BigDecimal(String.valueOf(queryArray[0].length() * 0.3)).setScale(0, BigDecimal.ROUND_HALF_UP);
        fuzzyCmd.add("edrec(\"" + queryArray[0] + "\", lower(title), " + String.valueOf(maxWrongWord)+ ") = 1 ");
        for (int i = 1; i < queryArray.length; ++ i) {
            BigDecimal maxWrongWord1 = new BigDecimal(String.valueOf(queryArray[i].length() * 0.3)).setScale(0, BigDecimal.ROUND_HALF_UP);
            fuzzyCmd.add("AND edrec(\"" + queryArray[i] + "\", lower(title), " + String.valueOf(maxWrongWord1)+ ") = 1 ");
    ...
    public List<Stars> listStarsByFuzzySearch(String query, int limit, int offset) {
        ...
        for (int i = 1; i < queryArray.length; ++ i) {
            BigDecimal maxWrongWord1 = new BigDecimal(String.valueOf(queryArray[i].length() * 0.3)).setScale(0, BigDecimal.ROUND_HALF_UP);
            fuzzyCmd.add("AND edrec(\"" + queryArray[i] + "\", lower(name), " + String.valueOf(maxWrongWord1)+ ") = 1 ");
