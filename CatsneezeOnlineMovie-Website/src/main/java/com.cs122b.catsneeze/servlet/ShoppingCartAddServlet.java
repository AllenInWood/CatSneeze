        ...
        if (session == null) {
            ...
            if (customerVo == null) {
                ...
                if (shoppingCart.isEmpty() || !shoppingCart.containsKey(movieId)) {
                    ...
                    shoppingItemVo.setMovieVo(serverResponse.getData());
                    ...
                    number += shoppingItemVo.getQuantity();
                    shoppingItemVo.setQuantity(number);
                    shoppingCart.put(movieId, shoppingItemVo);
                }
                session.setAttribute(Const.SHOPPING_CART, shoppingCart);
                String json = new Gson().toJson(ServerResponse.createBySuccessMessage("add item successfully"));
                response.setContentType("application/json");
                response.setCharacterEncoding("UTF-8");
                response.getWriter().write(json);
            }
